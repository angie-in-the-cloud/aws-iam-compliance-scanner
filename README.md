...
|**Root Account Activity**|Searches CloudTrail for root login events in the last 90 days|Root usage bypasses all IAM controls - it should never appear in logs|
...
```bash
aws s3 cp s3://your-iam-snapshot-bucket/reports/iam\_compliance\_snapshot\_<timestamp>.csv . --profile your-profile-name
```
...
```json
{
  "report\_metadata": {
    ...
    "generated\_at": "2026-04-14T00:00:00+00:00",
  ...
  "summary": {
    ...
    "compliant": 9,
    "non\_compliant": 3,
    "compliance\_score": "75.0%",
    "overall\_status": "NON\_COMPLIANT"
  },
  "findings": \[...]
}
```
...
|scanner|control|status|actual\_value|required\_value|nist\_control|soc2\_control|iso\_control|remediation|
|-|-|-|-|-|-|-|-|-|
|PasswordPolicy|MinimumPasswordLength|NON\_COMPLIANT|8|14|IA-5(1)(a)|CC6.6|A.9.4.3|Set minimum password length to at least 14 characters|
|MFAEnforcement|MFA enabled for jane.doe|NON\_COMPLIANT|No MFA device|MFA device required|IA-2(1)|CC6.1|A.9.4.2|Enable MFA for IAM user 'jane.doe'|
|RootActivity|Root account activity (last 90 days)|COMPLIANT|0 root events detected|0 root events|AC-2(5)|CC6.2|A.9.2.3|No action required|
